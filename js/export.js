(function () {
  ...
  async function exportSheet(sheet, lines) {
    ...
    aoa.push([]);
    aoa.push(['物料编码', '物料名称', '系统单位', '实盘单位', '规格信息', '仓库', '盘点数量', '备注']);
    for (const l of lines) {
      aoa.push([l.code, l.name, l.system_unit || l.unit || '', l.unit || '', l.spec, l.warehouse || '', l.qty, l.remark || '']);
    }
    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!cols'] = [{ wch: 14 }, { wch: 22 }, { wch: 10 }, { wch: 10 }, { wch: 24 }, { wch: 12 }, { wch: 12 }, { wch: 28 }];
